...
def get_image_paths(root_dir):
    """The original datasets contains the two part;
        - The first part is the original human image
        - The second part is the human matting part

        This function is to pair the image ans save their path
    """
    image_paths_dict = {}
    matting_paths_dict = {}
...
def split(image_paths, num_val_samples= 6000):
    """Split the datasets into  - train set : validation set = 8 : 2"""
...
def write_masks(image_paths, root_dir, add_mask_paths=True):
    ...
    for i, [image_path, matting_path] in enumerate(image_paths):
        ...
        ### Resize the image
        image_ = cv2.resize(matting_image, (300, 400))
        #cv2.imshow('Stretched Image', image_)
        cv2.imwrite(matting_path, image_)
        matting_image = cv2.imread(matting_path, -1)
